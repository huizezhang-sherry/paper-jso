...
data by revealing interesting projections. Optimization of some PP
...
The performance of JSO for visualizing data is evaluated across various
...
\(A\) that maximizes the index value of the projection, \(Y = XA\):
...
Here, the potential for a Jellyfish Search Optimizer (JSO) Rajwar, Deep,
and Das (2023) for the PPGT is explored. JSO, inspired by the search
behavior of jellyfish in the ocean, is a swarm-based metaheuristic
...
PP indexes are conducted to assess JSO's behavior and its sensitivity to
...
relatively easy to optimist. A goal is to develop PP indexes based on
...
\begin{tcolorbox}[enhanced jigsaw, left=2mm, bottomrule=.15mm, colback=white, colbacktitle=quarto-callout-note-color!10!white, titlerule=0mm, bottomtitle=1mm, opacityback=0, opacitybacktitle=0.6, coltitle=black, breakable, toptitle=1mm, colframe=quarto-callout-note-color-frame, toprule=.15mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, arc=.35mm, leftrule=.75mm, rightrule=.15mm]
...
optimisers to detect the structured projections. The same approach is
...
the optimiser did not come sufficiently close to the optimal projection.
...
number of tries) on the success rate is summarized in
...
with relatively lower dimensional (4, 6) search spaces, small parameter
values (20 jellyfish and a maximum of 50 tries) are sufficient to
achieve a high success rate. Many more jellyfish (100 jellyfish and a
maximum of 100 tries) are needed to obtain a mediocre success rate for
dimensions 10 and 12. This suggests that even JSO will struggle with
high dimensions, and require substantial more computational time to have
an acceptable success rate, especially for difficult PP indexes based on
scagnostics.
...
index which is noisier and the optimisation needs to be much closer to
...
Using the JSO optimiser for PPGT requires a change in user behavior. For
all the existing optimisers a single optimisation path is followed. The
JSO has multiple optimisation paths, and needs additional tools to
