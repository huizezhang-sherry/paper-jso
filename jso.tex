...
\end{keywords}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, boxrule=0pt, enhanced, sharp corners, interior hidden, frame hidden, breakable]}{\end{tcolorbox}}\fi
...
<<<<<<< HEAD
\begin{tcolorbox}[enhanced jigsaw, toptitle=1mm, opacitybacktitle=0.6, colframe=quarto-callout-note-color-frame, colback=white, breakable, colbacktitle=quarto-callout-note-color!10!white, bottomrule=.15mm, coltitle=black, opacityback=0, titlerule=0mm, leftrule=.75mm, bottomtitle=1mm, arc=.35mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, rightrule=.15mm, toprule=.15mm, left=2mm]
=======
\begin{tcolorbox}[enhanced jigsaw, breakable, rightrule=.15mm, leftrule=.75mm, opacitybacktitle=0.6, arc=.35mm, toptitle=1mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, colframe=quarto-callout-note-color-frame, toprule=.15mm, bottomrule=.15mm, coltitle=black, titlerule=0mm, bottomtitle=1mm, left=2mm, colbacktitle=quarto-callout-note-color!10!white, colback=white, opacityback=0]
>>>>>>> e54e8609eda22afb30d783cdb347f2b8e661e5ad
...
multiple tour paths are explored by the jellyfish, the animation
...
can then be extracted and viewed as a planned tour. The following code
uses JSO, with 20 jellyfish and a maximum of 50 tries, on the penguins
data using the \texttt{lda\_pp} index.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# clean up the penguins data}
\NormalTok{penguins\_cl }\OtherTok{\textless{}{-}}\NormalTok{ palmerpenguins}\SpecialCharTok{::}\NormalTok{penguins }\SpecialCharTok{|\textgreater{}} 
  \FunctionTok{filter}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(bill\_length\_mm)) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{rename}\NormalTok{(}\AttributeTok{bl =}\NormalTok{ bill\_length\_mm, }\AttributeTok{bd =}\NormalTok{ bill\_depth\_mm,}
         \AttributeTok{fl =}\NormalTok{ flipper\_length\_mm, }\AttributeTok{bm =}\NormalTok{ body\_mass\_g)}
\NormalTok{penguins\_df }\OtherTok{\textless{}{-}}\NormalTok{ penguins\_cl }\SpecialCharTok{|\textgreater{}} \FunctionTok{select}\NormalTok{(bl}\SpecialCharTok{:}\NormalTok{bm) }\SpecialCharTok{|\textgreater{}} \FunctionTok{rescale}\NormalTok{()}
\CommentTok{\# run the projection pursuit with JSO }
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{animate\_xy}\NormalTok{(penguins\_df, }
                  \FunctionTok{guided\_tour}\NormalTok{(}\FunctionTok{lda\_pp}\NormalTok{(}\AttributeTok{cl =}\NormalTok{ penguins\_cl}\SpecialCharTok{$}\NormalTok{species),}
                              \AttributeTok{search\_f =}\NormalTok{ search\_jellyfish, }
                              \AttributeTok{n\_jellies =} \DecValTok{20}\NormalTok{, }\AttributeTok{max.tries =} \DecValTok{50}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# extract the bases from the first jellyfish}
\NormalTok{bases }\OtherTok{\textless{}{-}}\NormalTok{ res }\SpecialCharTok{|\textgreater{}} \FunctionTok{filter}\NormalTok{(loop }\SpecialCharTok{==} \DecValTok{1}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{pull}\NormalTok{(basis) }\SpecialCharTok{|\textgreater{}} \FunctionTok{check\_dup}\NormalTok{(}\FloatTok{0.1}\NormalTok{)}
\CommentTok{\# visualise its path with a planned tour}
\FunctionTok{animate\_xy}\NormalTok{(}\AttributeTok{data =}\NormalTok{ penguins\_df, }\AttributeTok{tour\_path =} \FunctionTok{planned\_tour}\NormalTok{(bases), }
           \AttributeTok{col =}\NormalTok{ penguins\_cl}\SpecialCharTok{$}\NormalTok{species)}
\end{Highlighting}
\end{Shaded}

Figure~\ref{fig-jso-penguins} shows six frames from the tour jellyfish
No.1 animation. The Gentoo species (red) was mixed with Chinstrap
(yellow) initially and gradually separated out as flipper length
(\texttt{fl}) gaining more weights.

\begin{figure}

\centering{

\includegraphics{jso_files/figure-pdf/fig-jso-penguins-1.pdf}

}

\caption{\label{fig-jso-penguins}Visualising the tour path of a single
jellyfish (No.1) in the penguins data set. The projection pursuit is
optimised by the JSO with 20 jellyfishes and a maximum of 50 tries. The
bases visited by jellyfish No.1 is extracted and constructed as a
planned tour. The jellyfish successfully finds the structure of the data
...
{\centering \includegraphics{jso_files/figure-pdf/fig-jso-penguins-1.pdf}
...
\leavevmode\vadjust pre{\hypertarget{ref-posse1995}{}}%
