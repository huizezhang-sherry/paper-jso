...
\begin{tcolorbox}[enhanced jigsaw, opacitybacktitle=0.6, bottomtitle=1mm, colbacktitle=quarto-callout-note-color!10!white, left=2mm, toprule=.15mm, colback=white, leftrule=.75mm, arc=.35mm, opacityback=0, title={Algorithm: Jellyfish Optimizer Pseudo Code}, coltitle=black, rightrule=.15mm, toptitle=1mm, bottomrule=.15mm, breakable, titlerule=0mm, colframe=quarto-callout-note-color-frame]
...
hyper-parameters. This results in a total of 76 scenarios, comprising of
30 computed on the pipe data and 46 on the sine-wave data. A list of
combinations of the data shape, dimensions and PP indexes that are
...
\begin{longtable}[t]{>{\raggedright\arraybackslash}p{5em}rrrrrr}

\caption{\label{tbl-joint-output}Model summaries for squintability and
...
\\
\toprule
Model & Null Deviance & DF Null & Log-Likelihood & DF Residual & AIC & BIC\\
\midrule
\textbf{Squintability} & 49.29 & 75 & -26.44 & 71 & 62.87 & 74.53\\
\textbf{Smoothness} & 49.29 & 75 & -33.23 & 71 & 76.47 & 88.12\\
\bottomrule
...
Squintability and smoothness are likely dependent, making it unnecessary
to include both as independent factors in the model. Intuitively, a
highly smooth search space exhibits gradual changes, making it easier to
perceive the overall structure from a distance, thereby increasing the
squint angle. Table~\ref{tbl-joint-output} compare the model fits when
squintability and smoothness are included separately. Given that the
null deviance remains the same across models, the model with
squintability yields a smaller residual deviance and lower AIC and BIC
values, indicating a better fit. This suggests that squintability
captures the relevant variation more effectively than smoothness.
