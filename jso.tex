...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, opacityback=0, arc=.35mm, opacitybacktitle=0.6, left=2mm, colback=white, bottomtitle=1mm, titlerule=0mm, toptitle=1mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, bottomrule=.15mm, rightrule=.15mm, coltitle=black, toprule=.15mm, breakable, colbacktitle=quarto-callout-note-color!10!white, leftrule=.75mm]
...
show the quantile of index value in the 50 optimal projection, with 0
...
bootstrap replicates sampled from the simulation for each case. As the
number of jellyfish and maximum iteration allowed increase, the success
rate also increases. For problems in lower dimensional (4, 6) search
spaces, small parameter values (20 jellyfish and 50 iterations) are
sufficient to achieve a high success rate. Many more jellyfish (100
jellyfish and 100 iterations) are neede for finding the pipe structure
in d = 10, and 12. This suggests that even JSO will struggle with high
dimensions, and require substantial more computational time to have an
acceptable success rate, especially for difficult PP indexes based on
scagnostics.
...
self-standardized to range between 0 and 1, for comparison.) The
\texttt{splines} index is very smooth with a gradual increase in value
between the noise projection and the optimal projection. The
\texttt{TIC} index is slightly less smooth and slightly less gradual
change in value. Least desirable is the \texttt{skinny} index which is
noisier and the optimization needs to be much closer to the optimal
angle to have a higher value. This illustrates how the new measures for
squintability and smoothness work to describe the PP indexes.
...
resemble a needle-in-a-haystack problem. Additionally, if the index
function is differentiable, automatic differentiation can be used to
implement new gradient-based optimizers.
