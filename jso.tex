...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, titlerule=0mm, opacityback=0, coltitle=black, colback=white, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, arc=.35mm, breakable, toptitle=1mm, left=2mm, leftrule=.75mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, colbacktitle=quarto-callout-note-color!10!white, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm]
...
30 computed on the pipe data and 46 on the sine-wave data. This study
does not consider dimensions lower than four, as exploring the search
space in such cases is relatively straightforward. Similarly, extremely
high-dimensional cases with complex data structures, where no method can
reliably and consistenly uncover the underlying structure due to the
curse of dimensionality, are also excluded, as they fall outside the
scope of this study. A list of combinations of the data shape,
dimensions and PP indexes that are considered in this study is provided
in Table~\ref{tbl-smoothness-squintability}. Again, JSO is run 50 times
for each scenario to calculate the success rate for each hyper-parameter
combination.
