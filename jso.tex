...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, opacityback=0, left=2mm, toptitle=1mm, rightrule=.15mm, arc=.35mm, titlerule=0mm, coltitle=black, title={Algorithm: Jellyfish Optimizer Pseudo Code}, colbacktitle=quarto-callout-note-color!10!white, toprule=.15mm, bottomrule=.15mm, breakable, leftrule=.75mm, bottomtitle=1mm, opacitybacktitle=0.6, colback=white]
...
half-integer, the most popular values in the literature being
\(\frac{1}{2}\), \(\frac{3}{2}\) and \(\frac{5}{2}\). The parameter
\(\nu\), called \emph{smoothness parameter}, controls the decay of the
...
search optimization. There exist several R packages, such as
\texttt{GpGp} \citep{guinness2021gpgp} or \texttt{ExaGeoStatR}
\citep{abdulah2023large}, to fit the hyperparameters of a GP covariance
function on data. In this project, the \texttt{GpGp} package is used.
...
\(A^*\) be the optimal basis achieving the maximum index value for a
given projection pursuit problem defined by the shape to find, data
dimension, and the index function \(f(.)\). The projection distance
\(d(A, A^*)\), is defined as the Frobenius norm \(\lVert . \rVert _F\),
between the squared difference of these two bases:

\begin{equation}\protect\hypertarget{eq-proj-dist}{}{d(A, A^*) = \lVert M \rVert _F = \sum_{ij} M_{ij}^2, \text{ where } M = AA^\prime - A^*A^{*\prime}\ .}\label{eq-proj-dist}\end{equation}
