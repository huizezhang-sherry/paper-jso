...
\begin{tcolorbox}[enhanced jigsaw, colback=white, colframe=quarto-callout-note-color-frame, title={Algorithm: Jellyfish Optimizer Pseudo Code}, breakable, bottomrule=.15mm, toptitle=1mm, titlerule=0mm, opacityback=0, bottomtitle=1mm, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, coltitle=black, toprule=.15mm, leftrule=.75mm, left=2mm, opacitybacktitle=0.6, rightrule=.15mm]
...
examines the performance JSO under different JSO hyper-parameters
...
with JSO hyper-parameters, and data dimension, on the performance of JSO
...
projection distance contraint, (2) interpolate the sampled bases to the
optimal basis to calculate the projection distance and the index value.
(3) bin the index values by projection distances to obtain the average
index value for each bin, (4) fit the scaled sigmoid function in
equation \eqref{eq-squintability} to the binned index values against
projection distances using non-linear least square, 5) calculate the
squintability measure using equation (\ref{eq-squintability-parametric})
with parameters estimated from the model.}
