...
\begin{tcolorbox}[enhanced jigsaw, breakable, arc=.35mm, colframe=quarto-callout-note-color-frame, colback=white, rightrule=.15mm, coltitle=black, toprule=.15mm, toptitle=1mm, titlerule=0mm, colbacktitle=quarto-callout-note-color!10!white, opacityback=0, leftrule=.75mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, opacitybacktitle=0.6, bottomtitle=1mm, bottomrule=.15mm, left=2mm]
...
(\textbf{?@eq-squintability}) should provide a sensible measure of
...
To compute the squintability metric (\textbf{?@eq-squintability}) in
...
(\ref{eq-parametric}), the squintability metric
(\textbf{?@eq-squintability}) can be shown to be equal to
...
Alternatively, one can estimate (\textbf{?@eq-squintability}) in a
...
\textbf{?@eq-squintability} to the binned index values against
