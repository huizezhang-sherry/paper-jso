...
This study explores the potential for a Jellyfish Search Optimiser 
(JSO) to be integrated with projection 
pursuit guided tour. JSO [@chou_novel_2021] -[@rajwar_exhaustive_2023], 
inspired by the search behaviour of jellyfish 
in the ocean, is a swarm-based metaheuristic designed to solve global
optimisation problems. Compared to traditional methods, JSO 
demonstrated stronger search ability and faster convergence, and 
requires fewer tuning parameters. These
practical advantages make JSO as a promising candidate for enhancing
PP optimisation. As such, the study aims to explore JSO's potential
in enhancing projection pursuit and examines the characteristics
and advantages of such implementation.

The primary goal of this study is to investigate the performance of JSO
in PP optimisation. A series of simulation
experiments using well-recognised datasets and various PP indexes are conducted
to yield insights regarding the behaviour of JSO under different
settings and its sensitivity to hyper-parameter.
Second, to observe the performance of JSO with different types of PP
indexes, metrics are introduced to capture specific
...
smoothness within this context. Finally, the relationship between the JSO
...
optimisers and index functions in the literature. @sec-theory details JSO and
introduces metrics that measure different properties of PP indexes.
@sec-vis outlines the data structures, numerical summaries, and visualisations used in the simulation studies conducted in @sec-app to assess JSO's perforamnce.
@sec-app presents two sets of simulation
