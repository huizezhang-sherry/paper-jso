...
projection pursuit problems. This section outlines two simulation setups
to demonstrate the performance of JSO. In the first example, the 
performance of JSO is compared with an exist optimiser, Creeping Random Search (CRS), and explores how JSO behaves
...
along with JSO hyper-parameters, on the outcome (success rate) of the
optimisation.

The data structure proposed in @RJ-2021-105 is used here for assessing JSO. A sample for the sine-wave structure in 6D data using a spline index function (`splines2d`) is printed below:

```{r}
dplyr::glimpse(sim_sine_6d_spline_head |> select(-id, -info)) 
```

The JSO hyper-parameters used are 50 jellyfishes (`n_jellies`) and
...
jellyfishes, and 2) 50 and 100 maximum number of tries. The success 
...
procedures outlined in @sec-smoothness and @sec-squintability and illustrated in @fig-smoothness and  @fig-squintability. To
...
#| fig.height: 6
#| fig.width: 10
#| fig.cap: Best projections found by the JSO in each of the 50 simulations, sorted by index values. The projection pursuit problem is to detect the 6D sine-wave structure using the spline index. Most simulations have detected a clear sine-wave pattern, while the last six show a straight line with clustering at the ends. In this simulation, the success rate is 0.88 (44/50).
...
#| fig.cap: "Illustration of steps to calculate smoothness. "
...
one table. Three pre-processing steps are applied to the data: 1) scaling
...
projection, none of the 50 simulations identified the sine-wave shape. A
generalised linear model with a binomial family and a logit link
function is used to fit the data and @tbl-mod-output presents the model
