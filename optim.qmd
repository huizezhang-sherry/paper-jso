...
execution of the optimisation, we choose to focus here on the first two properties, 
namely *smoothness* (@sec-smoothness) and *squintability* (@sec-squintability), and propose two metrics to evaluate them.
...
It is expected that for a PP index with high squint angle, the optimization
...
differentiable), and the projection distances for which these values are attained,
...
represents the projection distance at which this largest gradient is attained.
...
To the best of our knowledge, this is the first attempt to measure mathematically the
