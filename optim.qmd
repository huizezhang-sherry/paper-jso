...
parameters and the optimisation properties defined in @sec-theory.
...
(`dcor2d_2`, `loess2d`, `MIC`, `TIC`, `spline`, and `stringy`) are
considered. Combined with the jellyfish optimisers (`n_jellies` =
20/50/100, `max_tries` = 50/100), we obtain additional 27 setups for
this investigation.

```{r}
load(here::here("data/smoothness.rda"))
smoothness |> select(index, n, variance:nugget) |> knitr::kable(digits = 3)
load(here::here("data/squintability.rda"))
squintability |> knitr::kable(digits = 3)
```
