...
## Performance of the jellyfish optimiser in high-dimensional pipe-finding problems

[unfinished]

The aim of this example is to investigate the performance of jellyfish optimisers in high-dimensional problems.

The projection pursuit problem is to find the pipe shape, investigated in @laa_using_2020, using the holes index in 6/8/10/12-D data. For each case, 50 simulations are conducted for each combination of jellyfish hyper-parameter: `n_jellies = 20/50/100` and `max_tries = 50/100`. The performance of the jellyfish optimiser is also compared with an existing opitmiser: the search better opitmiser with `max.tries = 1000`.

@fig-proportion presents the proportion of jellyfishes that finds the index value close to (absolute value less than 0.05) the best index value found at each `max_tries`/`n_jellies` combination at $d = 6/8/10/12$ data dimension. As the number of jellies and maximum tries increase, the proportion of jellyfishes that find the optimal projection increases. This suggests for higher dimensional problems ($d = 8/10/12$), an `n_jellies = 100` and `max_tries = 100` is necessary for at least half of the jellyfish optimiser to find the optimal projection.

@fig-proj presents the final projection found across the 50 simulations at each 10th quantile between the jellyfish and search better optimiser (`n_jellies = 100`, `max_tries = 100` is used for the jellyfish optimiser). While the jellyfish optimiser may not always find the optimal projection in the worse case scenarios, especially as the dimension increases, it tends to find higher index values and crisper pipe shapes than the search better optimiser at each quantile across all the four data dimensions. 
...
#| fig.height = 3,
#| fig.align = "center",
#| fig.cap = "Proportion of jellyfishes reaches close to the optimal index value in the pipe-finding problem. As the dimension goes up, the numer/ proportion of jellyfishes that reach the optimal index value decreases. Increasing the jellyfish hyperparameters (`n_jellies` and `max_tries`) help to improve the proportion."
pipe_run_summ <- sim_pipe |>
  mutate(idx_f = "holes") |>
  group_by(id, n_jellies, max_tries, d, loop) |> 
...
#| label = fig-proj,
#| fig.align = "center",
#| fig.cap = "Projections found by the jellyfish and search better optimiser at each 10th quantile across 50 simulations. The holes index is used to find the target pipe shape in the 6/8/10/12D space. An `n_jellies = 100` and `max_tries = 100` are used for the jellyfish optimiser and `max.tries = 1000` is used for the search better optimiser." 
