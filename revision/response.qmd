...
**\textcolor{purple}{Squint angle is the core concept inspires the metric squintability, hence we provide a separate definition of it in Definition 4. For all the projection bases in the space, those near the optimal basis correspond to projections with interesting structure we're reesarching. While for different indexes, the size of the neighbourhood is different: some indexes may have projections resemble the target even when it is far away, which corresponds to a large squint angle and easy to optimize (since it is easy for the optimizer to sample into the neighbourhood). To quantify how close projections need be before structures of interest can be observed (the optimizer starts hill-climbing), we can track how the index value changes as we move from an initial projection basis to the optimal one (projection distance). This prompts the definition of projection distance in Definition 5 and squintability in Definition 6. We have revise the text to explain this connection more clearly in Section 3.2.}**
...
Finally I have also collected a few minor remarks: 
  ...
  * After defining Eq. 4 the additional parameter should probably be theta_4 and not theta_2.  **\textcolor{purple}{TODO: Nicolas}**
  * In the algorithm pseudo-code it is not clear what c_t and the trend actually are.  **\textcolor{purple}{In the text and the pseudo-code, we include information that $c_t$ is the time control function determine the exploration and exploitation phase of the algorithm. We also include information that the (ocean) trend is defined as the difference between the best jellyfish and the average of all current jellyfish.}**
  * In Fig. 4 please specify what optimizer was used. **\textcolor{purple}{We have included the information in the caption that the CRS optimised is used for this illustration. }**
