...
**\textcolor{blue}{The correlation between the two variables are 0.55, which should not affect the modelling. (TODO: Di - do you mind checking if we need more here?) We did try CRS for the same sine-wave optimisation reported in Section 6, but most runs yield a failed result with the optimum not found by CRS, hence not included.}**
...
\textcolor{blue}{Table 1 is a subset of the PP index estimated and it happens that the three chosen ones all decrease as n goes larger. This is not necessary the case in the full table shown in the Appendix. The caption for Table 1 has been fixed.}
