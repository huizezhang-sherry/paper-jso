...
\begin{abstract}
The projection pursuit (PP) guided tour interactively optimizes a
...
optimising PP indexes. The performance of JSO is evaluated across
various hyper-parameter settings and compared with existing optimisers.
...
the \texttt{tourr} package, while functions to calculate smoothness and
squintability of indexes are available in the \texttt{ferrn} package.
\end{abstract}
...
to be integrated with the projection pursuit guided tour. JSO
...
practical advantages make JSO as a promising candidate for enhancing PP
optimisation. As such, the study aims to explore JSO's potential in
enhancing projection pursuit and examines the characteristics and
...
\begin{tcolorbox}[enhanced jigsaw, left=2mm, rightrule=.15mm, colframe=quarto-callout-note-color-frame, breakable, opacityback=0, title={Algorithm: Jellyfish Optimizer Pseudo Code}, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, bottomtitle=1mm, coltitle=black, titlerule=0mm, toptitle=1mm, arc=.35mm, bottomrule=.15mm, opacitybacktitle=0.6, leftrule=.75mm, colback=white]
