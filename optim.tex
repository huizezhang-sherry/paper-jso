...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, opacitybacktitle=0.6, titlerule=0mm, title={Algorithm: Jellyfish Optimizer Pseudo Code}, colback=white, bottomtitle=1mm, breakable, toprule=.15mm, bottomrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, rightrule=.15mm, toptitle=1mm, coltitle=black, leftrule=.75mm, left=2mm, arc=.35mm]
...
Section~\ref{sec-theory}, along with JS hyper-parameters, on the outcome
of the optimisation.

\subsection{Performance of JS optimiser in pipe-finding
problems}\label{performance-of-js-optimiser-in-pipe-finding-problems}
...
optimiser always find a clear pipe shape while the search better
optimiser also find the pipe shape but with a wide rim. At higher data
dimension, the JS optimiser always find a higher index value and a
clearer pipe shape across all the quantiles.}
...
simulation, success is defined as achieving a final index value within
0.05 of the best index value found among all 50 simulations. For
example, in the 8-dimensional data scenario with 20 jellyfishes and a
maximum number of tries set to 100, the optimal index value is 0.997.
Out of the 50 simulations, 27 achieved an index value within {[}0.947,
0.997{]}, resulting in a success rate of 0.54.
Figure~\ref{fig-proportion} presents the proportion of success for each
hyper-parameter combinations. As the number of jellyfish and maximum
tries increase, the success rate also increases. For simpler problems (6
dimensions), small parameter values (20 jellyfishes and a max number of
tries of 50) can already result in high success rate, while for
