...
# Calculated smoothness and squintability metrics for all PP indexes and data sets

Smoothness and squintability are metrics used to characterize the index 
function for PP tasks and are invariant to the optimizer and its 
hyper-parameters. @tbl-smoothness-squintability presents the Gaussian process
parameters (outputscale $\eta$, lengthscale $\ell$, smoothness $\nu$, and 
nugget $\sigma$) and scaled logistic function parameters ($\theta_1$ to 
$\theta_4$ and $\varsigma$) for all 23 PP problems investigated in our work. 
The columns $\nu$ and $\varsigma$ represent the smoothness and squintability 
...
dt <- tibble(shape = c(rep("pipe", 5), rep("sine", 18)), smoothness) |>
  ...
  mutate(index = as.character(index),
         index = ifelse(index == "dcor2d_2", "dcor", index),
