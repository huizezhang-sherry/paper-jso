...
\begin{equation}
\varsigma(f) = -4 \times \min_{d} g'(d) \times \arg \min_{d} g'(d)
\label{eq-squintability}
\end{equation}

where $4$ is a constant scaling factor, $-\min_d g'(d)$ represents the 
largest gradient of $-g$ and $\arg \min_{d} g'(d)$ represents the projection distance at which this largest gradient is attained. If the set $\mathcal{D}:=\arg \min_{d} g'(d)$ contains more than one value $d$, the largest value $\max(\mathcal{D})$ is used in equation \eqref{eq-squintability}.
...
\begin{proof}
Let $d_1 := \underset{d}{\arg \min }\,g_1'(d)$ and $d_2 := \underset{d}{\arg \min }\,g_2'(d)$.
Since $g_1$ is convex, $g_1''(d) >= 0 \; \forall d \in [0, D]$, meaning that $g_1'$ is an increasing function, taking its minimum for $d_1=0$. As a result, $\varsigma(g_1)=-4 \times g'(0) \times 0 =0$.
Since $g_2$ is concave, $g_2''(d) <= 0 \; \forall d \in [0, D]$, meaning that $g_2'$ is a decreasing function, taking its minimum for $d_2=D>0$. Since $g_2$ is strictly decreasing, $g_2'(d) < 0 \; \forall d \in [0, D]$. In particular $g_2'(D)<0$. As a result $\varsigma(g_2)=-4 \times g'(D) \times D >0$. This proves that $\varsigma(g_1) < \varsigma(g_2)$.
\end{proof}
