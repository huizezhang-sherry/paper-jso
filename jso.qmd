...
the initialization stage of JSO, then these random index values can be
...
then two close projection angles should produce close index values
(strong correlation); by contrast, if a PP index is not very smooth,
then two close projection angles might give very different index values
(fast decay of correlations with respect to distance between angles).
...
values in the literature being $\frac{1}{2}$, $\frac{3}{2}$ and
$\frac{5}{2}$. The parameter $\nu$, called *smoothness parameter*,
controls the decay of the covariance function. As such, it is an
appropriate measure of smoothness of a random field.
...
hyperparameters of a GP covariance function on data. In this project,
the `GpGp` package is used.
...
\begin{defn}\label{def:proj-dist}
...
\begin{defn}\label{def:squintability}
Let $g$ be the function that maps the projection distance, $d = d(A, A^*)$, 
to the index value $f(XA)$, such that $g(d(A, A^*)) = f(XA)$.
The squintability of $f(\cdot)$, denoted $\varsigma(f)$, is defined as 
$$\varsigma(f) = c \times \max_{d} \Delta g(d) \times \arg \max_{d}  \Delta g(d)$$ 
where $c$ is a constant scaling factor, $\max_A \Delta g(x)$ represents the 
maximum change in the index value along the curve $g$, and $\arg \max_{d} \Delta g(x)$ represents the projection distance at which this largest gradient is attained.

\end{defn}
Definition \ref{def:squintability} is ...
...
which this largest gradient is attained. It is expected that these two
