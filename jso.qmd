...
summary_table <- bind_rows(
  ...
  select(Model, deviance, df.residual, null.deviance, df.null, #logLik, 
         ) %>% #, AIC, BIC) %>%
  rename(
...
summary_table %>%
  kable(caption = "Fit statistics for models: full, individual squintability and smoothness. The full model (and the model containing squintability but not smoothness) explains more than 50% of the variability in optimisation success.") %>%
  kable_styling(font_size = 10) %>%
