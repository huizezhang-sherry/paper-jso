...
author:
  ...
  - name: Nicolas  Langrené
    affiliations:
        - id: 3
          name: Guangdong Provincial/Zhuhai Key Laboratory of Interdisciplinary Research and Application for Data Science, Beijing Normal-Hong Kong Baptist University
...
$f(XA): \mathbb{R}^{n \times d} \to \mathbb{R}$ defines a statistic to
...
#| fig-alt: "The figure contains four panels. Panels (a) and (c) are polar coordinate plots showing the values of projection pursuit (PP) indexes—skewness for trimodal data in (a) and binned normality for RANDU data in (c)—across all 1D projections of 2D datasets. The radius represents the index value for each projection angle, with a dashed circle indicating the average index and a straight line marking the optimal projection. Panel (a) displays a smooth but non-convex skewness index with three pronounced modes and a wide squint angle, while panel (c) shows a noisier, non-normality index with less smoothness and a narrow squint angle. Panels (b) and (d) are histograms representing data projected onto the optimal directions identified in (a) and (c), respectively; panel (b) reveals trimodality and panel (d) shows the discrete nature of the RANDU data, which is harder to discern except near the optimal projection."
...
graph elements (area, diameter, perimeter, and length) of alpha hull or
...
The squintability metric is inspired by the concept of squint angle in Definition \ref{def:squint-angle}, originally introduced in @barnett1981interpreting and later discussed in @laa_using_2020. Consider the space of all projection bases: those near the optimal basis correspond to projections with structure of interest. The size of this neighborhood depends on the choice of the index function. For some indexes, projections resemble the target even when relatively far from the optimum, corresponding to a large squint angle, while for others, projections need to be close to the optimum to reveal structure we are interested, resulting in a small squint angle. For PP, a small squint angle is considered to be undesirable because it means that the optimizer needs to sample into a very narrow region to be able to "see" the optimum, making it difficult for the optimizer to find the optimum. 
...
#| fig-alt: "The figure presents four line graphs, each showing simulated optimization traces for different projection pursuit (PP) index functions: MIC, splines2d, skinny, and stringy2. The x-axis likely represents optimization iterations or search steps, while the y-axis shows the index value. Each trace illustrates how rapidly the index value improves during optimization. The MIC and stringy2 functions display steeper early increases (representing high squintability or high squint angle), indicating fast progress toward the optimal projection. In contrast, splines2d and skinny traces rise more slowly (lower squintability), reflecting that substantial improvement is only achieved when candidate projections get very close to optimal. The chart visually supports the concept of squintability as the percent improvement at the halfway point of the index value."
sq_sine_basis_df |> 
  filter(n == 4) |> 
  ...
  mutate(value = (value - min(value)) / (max(value) - min(value)),
         index = fct_relevel(index, c("stringy2", "skinny", "MIC", "splines2d"))) |>
  ggplot(aes(x = dist, y = value, 
             group = interaction(index, id, n), 
             color = index)) +
  geom_line() + 
  colorspace::scale_color_discrete_sequential(palette = "RdPu", name = "Index function", 
                                              c2 = 100, rev = FALSE) + 
...
Let $g: \mathbb{R} \mapsto  \mathbb{R}$ be a decreasing function that models the index value $f(XA)$ as a function of the projection distance $r(A, A^*)$, such that $g(r) = g(r(A, A^*)) \simeq f(XA)$.  The squintability of an index function $f$ is then defined by 
...
iteration and terminates if no better projection is found within these 
...
#| fig.cap: "How success rate is calculated, illustrated using the optimal projections from 50 optimizations of 8D pipe data, optimized by CRS, sorted by index value. The pipe shape is recognizable in the projection index values between 0.933-0.969. Of the 50 simulations, 43 achieved an index value within 0.05 of the best, resulting in a success rate of 0.86."
#| fig-alt: "The figure compares histograms or scatterplots of projected data from optimization simulations, illustrating results when the final projection index is within 0.05 of the optimal value. Axes likely represent the values along the projected data component. In these projections, the underlying 'pipe' structure is not visibly distinguishable, supporting the threshold choice for the success rate metric. The visual suggests that projections within this index margin do not reveal meaningful structure in the data, validating the 0.05 threshold as a criterion for optimization success."
#| fig-pos: "!ht"
...
`r nrow(sim_df)` scenarios, comprising
...
#| fig-alt: "The figure depicts a flowchart or schematic illustrating the smoothness computation process for projection pursuit (PP) indexes. It shows that for each PP task, 500 random basis directions are generated, index values are computed for these projections, and a Gaussian process model is fitted to the resulting data. The final output is a quantified smoothness measure of the index, capturing how smoothly the index value changes across different basis projections."
...
#| fig-alt: "The diagram illustrates the procedure for calculating squintability of a projection pursuit (PP) index: 50 random initial bases are generated for each task, and linearly interpolated to the optimal basis in small increments of 0.005. For each interpolated basis, index values are computed and averaged within each 0.005 distance bin. These averaged values are then used to fit a logistic function via non-linear least squares, from which the squintability measure is derived, reflecting how quickly the index value improves as projections approach optimality."
...
averaged over the distance bin of 0.005. The logistic function in
...
This section summarizes the findings from the simulations that compare
...
#| fig-alt: "The figure presents a series of regression plots or bar charts depicting the relationships among squintability, smoothness, data dimension, and optimization success rate. The primary trend shows a moderate positive correlation between squintability and smoothness, but squintability is a much stronger predictor of success rate than smoothness. Addition of interaction terms with data dimension results in slight model improvement. Visual evidence suggests that higher squintability is strongly associated with higher success rates, while smoothness has only a minor influence. Differences between optimization algorithms, such as JSO and CRS, are noted, but optimizer efficiency is not strongly tied to smoothness."
...
#| fig-alt: "A grouped bar chart shows the success rate of the JSO optimizer for projection pursuit tasks across varying data dimensions (d = 4, 6, 8, 10, and 12), number of jellyfish (20, 50, 100), and maximum iterations (50, 100). Each bar’s height indicates the mean success rate, with error bars representing uncertainty from bootstrap resampling. For low-dimensional tasks (d = 4, 6), high success rates are achieved with few jellyfish and iterations, while higher dimensions (d = 10, 12) require significantly more jellyfish and iterations to approach similar rates. The trend highlights that as dimensionality increases, optimizing becomes harder and more computational resources are necessary for acceptable performance."
...
res_quantile |> 
  ...
  ggplot(aes(x = n_jellies - 0.5, y = proportion, 
             group = d)) + 
  geom_segment(aes(x = n_jellies - 1, xend = n_jellies + 1, color = d), 
               data = res_quantile |> summarise(proportion = mean(proportion)), 
               size = 2) + 
  geom_segment(aes(xend = n_jellies + 0.5, color = d), size = 0.05) + 
  geom_line(aes(x = n_jellies, color = d),  
            data = res_quantile |> summarise(proportion = mean(proportion)), 
            alpha = 0.5, show.legend = FALSE) +
  scale_color_manual(values = colorspace::sequential_hcl(5, "RdPu", c2 = 200),
                     name = "Dimension", labels = seq(4, 12, 2)) + 
  # scale_linetype_manual(values = c(1, 5,2,4,3), guide = "legend", 
  #                       name = "Dimension", labels = seq(4, 12, 2)) + 
  scale_x_continuous(breaks = c(20, 50, 100), labels = c("20", "50", "100")) +
  facet_wrap(vars(`max # of iteration`), labeller = "label_both") + 
  theme_bw() +
...
#| tbl-cap: The columns $\nu$ and $\varsigma$ show the smoothness and squintability measures for PP indexes for the sine wave data. Only three are shown for illustration, and a full table can be found in Supplementary materials. The splines index has the largest squint angle and is the smoothest. The skinny index has low smoothness and smallest squint angle. 
...
#| fig-alt: "A line chart displays the self-standardized PP index values (ranging from 0 to 1 on the y-axis) plotted along the interpolation path from a random noise projection to the optimal projection (x-axis) for 6D sine wave data. Three index traces—splines, TIC, and skinny—are compared: the splines index line rises smoothly and steadily, indicating high smoothness and gradual improvement; the TIC index shows moderate smoothness with a slightly steeper increase; the skinny index trace is noticeably jagged, remaining low until near the optimal projection where it sharply increases. These patterns demonstrate the differences in squintability and smoothness among the PP indexes, with smoothness and gradual improvement aiding optimization."
sq_basis_df_6d_TIC_spline_skinny |>
  ggplot(aes(x = dist, y = index, group = interaction(index_name, id), 
             color = fct_relevel(index_name, c("skinny", "TIC", "splines")))) +
  geom_line() + 
  colorspace::scale_color_discrete_sequential(palette = "RdPu", name = "Index function", 
                                              c2 = 100, rev = FALSE) + 
  xlab("Projection distance") + ylab("Index value") +
...
bases recorded and construct it as a planned tour using the `planned_tour()` function (see the examples via `?search_jellyfish`).
Users may wish to view the optimization path of the most successful jellyfish, or a random selection of jellyfish, 
or some specific runs that find local maxima.
...
Provincial/Zhuhai Key Laboratory IRADS (2022B1212010006) and the BNBU
...
the indexes used in the simulation study, 2) the table of the Gaussian
process parameters and logistic function parameters for all 23 PP problems investigated, 3) the script to get started with using JSO in a PPGT and calculating smoothness and squintability,
as explained in @sec-discussion, along with 4) the full code to reproduce the plots and summaries in this paper.
