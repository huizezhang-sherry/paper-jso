...
angle). This suggests that their product (@eq-squintability) should
...
Let $g_1(d)$ be a convex function and $g_2(d)$ be a concave function, where $d \in [0, D]$ and $g_1(d), g_2(d) \in [0, 1]$. Assume both functions achieve the same maximum gradient, i.e., $\max g'_1(d_1) = \max g'_2(d_2) = -g_{max}$, it follows that $d_1 < d_2$ and $\varsigma(f_1) < \varsigma(f_2)$.
\end{prop}
