...
library(kableExtra)
mod2 <- glm(proportion ~ squintability + d + n_jellies + max_tries, data = sim_df2, family = binomial)
mod3 <- glm(proportion ~ smoothness + d + n_jellies + max_tries, data = sim_df2, family = binomial)
summary_table <- bind_rows(
  glance(mod2) %>% mutate(Model = "Squintability"),
  glance(mod3) %>% mutate(Model = "Smoothness")
) %>%
  select(Model, null.deviance, df.null, logLik, df.residual, AIC, BIC) %>%
  rename(
