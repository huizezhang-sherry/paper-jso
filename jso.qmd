...
#| out.width: 100%
#| fig.cap: |
#|   Steps for calculating squintability in a projection pursuit problem. Given the target shape, data dimension and the index function: (1) sample random bases given the orthonormality and projection distance contraint, (2) interpolate the sampled bases to the optimal basis to calculate the projection distance and the index value. (3) bin the index values by projection distances to obtain the average index value for each bin, (4) fit the scaled sigmoid function in equation \eqref{eq-squintability} to the binned index values against projection distances using non-linear least square, 5) calculate the squintability measure using equation (@eq-squintability-parametric) with parameters estimated from the model.
